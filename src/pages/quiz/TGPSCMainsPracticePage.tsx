...
interface TGPSCMainsPracticePageProps {
  initialData?: any;
}

const TGPSCMainsPracticePage: React.FC<TGPSCMainsPracticePageProps> = ({ initialData }) => {
  ...
  useEffect(() => {
    ...
    if (initialData && initialData.quizzes) {
      const tgpscQuizzes = initialData.quizzes.filter((quiz: Quiz) => 
        quiz.quizType === 'mainsPractice' && quiz.examBoard === 'tgpsc'
      );
  ...
  useEffect(() => {
    ...
    const fetchQuizzes = async () => {
      try {
        // Query quizzes with quizType = 'mainsPractice' and examBoard = 'tgpsc'
        const q = query(
      ...
      <div className="max-w-7xl mx-auto">
        <div className="text-center mb-12">
          <h1 className="text-3xl font-extrabold text-gray-900 sm:text-4xl">
            TGPSC Mains Practice
          </h1>
          <p className="mt-3 max-w-2xl mx-auto text-xl text-gray-500 sm:mt-4">
            Practice with detailed explanations for TGPSC Mains preparation
          </p>
        </div>
        ...
        <div className="mb-8 bg-white p-4 rounded-lg shadow-sm border border-gray-200">
          ...
          {dateFilter !== 'all' && (
            <div className="mt-3 text-sm text-gray-500 text-center sm:text-right">
              Showing {filteredQuizzes.length} {filteredQuizzes.length === 1 ? 'quiz' : 'quizzes'} from {dateFilter === 'today' ? 'today' : dateFilter === 'week' ? 'the past 7 days' : dateFilter === 'month' ? 'the past 30 days' : 'the selected range'}
            </div>
          )}
        </div>
        ...
        {filteredQuizzes.length === 0 ? (
          <div className="text-center py-12">
            <p className="text-lg text-gray-600">No quizzes available {dateFilter !== 'all' ? 'for the selected date range' : 'at the moment'}. Please check back later.</p>
          </div>
        ) : (
...
export default TGPSCMainsPracticePage;
