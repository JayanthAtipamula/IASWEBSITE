...
interface TGPSCPrelimsPracticePageProps {
  initialData?: any;
}

const TGPSCPrelimsPracticePage: React.FC<TGPSCPrelimsPracticePageProps> = ({ initialData }) => {
  ...
  useEffect(() => {
    ...
    if (initialData && initialData.quizzes) {
      const tgpscQuizzes = initialData.quizzes.filter((quiz: Quiz) => 
        quiz.quizType === 'prelimsPractice' && quiz.examBoard === 'tgpsc'
      );
